...
for item in audit['pendingMatches']:
 ...
 known=f"本批 USD {item['priceUSD']:.2f}/件、MOQ {item['moq']}；{item['format']}。对照记录：USD {old['unitPrice']:.2f}/件、MOQ {old['moq']}；容量以对应规格为准。已确认同类型/同系列但有不同香味。"
 if item['possibleExistingId']=='BC-01':known+=' BC-01独立确认为500mL、USD3.00、MOQ6。B05-01保留USD2.99。'
 add('MATCH-'+item['intakeId'],'同款关系',item['name']+'：'+item['intakeId']+' ↔ '+item['possibleExistingId'],known,'请逐侧填写具体香味名称、对应SKU及图片文件名。无需再次确认容量或是否同系列。'+item['reason'],'仅对应香味产品；补齐香味/图片对应前保持独立记录，不混用图库或报价。',[item['gallery'][0]['thumbnail'],old['image']])
items=[
...
('MAIL-SERVICE','邮件配置','Resend及发件域','Vercel中已有RESEND_API_KEY和INQUIRY_FROM_EMAIL（Preview/Production Secret），不能导出；Resend网页未登录。Preview禁止真实发送。','请在Resend确认账号/发件域验证状态；表格仅填已配置/待配置及公开发件邮箱，不填API Key。','影响表单实际发信，不阻塞目录预览'),
('MAIL-ENV','邮件配置','正式环境配置','变量：RESEND_API_KEY、INQUIRY_FROM_EMAIL、INQUIRY_TO_EMAIL、INQUIRY_ENABLED。','现有Secret已存在，无需重复提交。先在Resend核对发件域与发件地址；正式发布且邮件验收通过后才启用发送。此处仅记录核对状态。','影响正式邮件表单'),
('MAIL-TEST','邮件配置','真实收发验证','成功/失败逻辑仅经模拟测试，未验证真实送达、垃圾箱及回复。','配置完成后请填可用于验证的接收安排及是否授权发送一封测试邮件；拟在另行授权后发一封标注TEST的邮件到已确认销售邮箱，核对收件/垃圾箱及回复；本轮不发送。','影响正式邮件链路验收'),
...
confirmed_ids={'CONTACT-WA','CONTACT-MAIL','SHIP-ORIGIN','SHIP-TIME','SHIP-FREIGHT','SHIP-TAX','RETURN','PAYMENT','QUOTE'}
for row in items:
 if row[0] not in confirmed_ids:add(*row)
for item in audit.get('materialReplacementGroups',[]):
 add('IMAGE-'+item['ids'][0],'容量素材',item['name'],f"已确认：{item['confirmedSize']}；现有图显示旧容量 {item['oldLabel']}。",'请提供当前容量真实产品正面/背面及标签照片，注明各图片对应编号；不要修改旧图瓶身标签数字。','仅影响相应产品素材的正式发布；数值与订单已按确认信息更新。',item['images'])
...
for item in allrows:
 fields=[s for s in item['missing'] if not any(t in s.lower() for t in ['capacity','net contents','容量','moq']) and not ('shipping' in s.lower() or 'dispatch' in s.lower() or 'lead-time' in s.lower())]
 if any('stock' in s.lower() for s in item['missing']):fields.append('Availability for the specific requested order; confirmed individually')
 if not fields:continue
 add('DOC-'+item['ids'][0],'其他产品资料',item['name'], '编号：'+', '.join(item['ids'])+'；当前容量：'+item['format']+'；USD '+str(item.get('unitPrice','待确认'))+'/件；MOQ '+str(item.get('moq','待确认'))+'件。','仅补尚缺资料：'+'；'.join(fields)+'。可填写资料文件名及对应编号；未取得可填待供应商提供。','对应产品的规格/宣传/最终报价，不阻塞其他产品。',[item['image']])
def img(src):
...
print(json.dumps({'rows':len(rows),'capacity':len(audit['capacity']),'matches':len(audit['pendingMatches']),'fileBytes':len(page.encode())}))
