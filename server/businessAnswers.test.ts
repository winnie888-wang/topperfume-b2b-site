...
const sourceProduct = (slug: string) => allProducts.find(p => p.slug === slug)!;
...
describe("Business answers and explicit scope corrections", () => {
  it("changes only the ten explicitly repriced products; preserves all images and blank answers", () => {
    ...
    for (const before of history.products) {
      const current = sourceProduct(before.slug);
    ...
    expect(sourceProduct("vanilla-cashmere-body-wash-473ml").unitPrice).toBe(4.99);
    expect(sourceProduct("vitamin-c-body-lotion-502ml").unitPrice).toBe(2.99);
  });
  ...
  it("replaces GlutaGlow sizes without a third option and rejects retired order variants", () => {
    ...
    const current = sourceProduct(slug);
    ...
    for (const suffix of ["", "?variant=B04-01-400", "?variant=B04-01-600"]) expect(() => resolveInquiryProduct({ ...base, productUrl: `https://topperfume.cn/products/${slug}${suffix}` })).toThrow(/catalogue/);
    const inquiry = resolveInquiryProduct({ ...base, productUrl: "https://topperfume.cn/products/vanilla-cashmere-whipped-oil-body-butter-283g" });
    expect(inquiry).toMatchObject({ format: "283 g", unitPrice: "US$3.00 / piece", subtotal: "US$18.00" });
    const text = new URL(getWhatsAppCtaUrl({ intent: "quote", context: inquiry })).searchParams.get("text")!;
    for (const term of ["283 g", "US$3.00", "US$18.00", "excludes shipping and taxes"]) expect(text).toContain(term);
  });

  it("changes precisely four group-photo variants, preserving the two other 200 mL products", () => {
    const p = sourceProduct("vaseline-body-care-series-200ml");
    expect(getProduct(p.slug)).toBeUndefined();
    ...
    for (const variant of p.variants!) {
      expect(() => resolveInquiryProduct({ intent: "quote", quantity: "6", countryMarket: "QA", customerName: "Test only", customerEmail: "test@example.invalid", productUrl: `https://topperfume.cn/products/${p.slug}?variant=${variant.id}` })).toThrow(/catalogue/);
    }
  });
