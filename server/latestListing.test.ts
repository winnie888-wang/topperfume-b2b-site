...
import { allProducts, getProduct } from "../client/src/data/products";
...
const allSourceIds = allProducts.flatMap(p => p.intakeIds ?? []);
...
describe("Batch 04–05 identity and scoped quotes", () => {
  ...
  it("keeps six source records and their quotes while excluding their held public pages", () => {
    ...
    for (const product of latestListingProducts) {
      ...
      expect(html).not.toContain(`US$${(price * 6).toFixed(2)}`);
    ...
    expect(sourceProduct("cocoa-radiant-body-gel-oil").format).toBe("237 mL");
    expect(sourceProduct("calm-healing-body-lotion").format).toBe("500 mL");
  });

  it("replaces both GlutaGlow sizes with a single 725 mL offer and preserves other lotions", () => {
    const product = sourceProduct("glutaglow-body-lotion-400ml-600ml");
    ...
    expect(sourceProduct("gluta-glow-body-lotion-500ml").minimumOrderQuantity).toBe(6);
    expect(sourceProduct("vitamin-c-body-lotion-502ml").minimumOrderQuantity).toBe(2);
  ...
  it("does not create duplicate catalogue entries or reprice old variants for five unresolved matches", () => {
    ...
    expect(sourceProduct("daily-niacinamide-body-lotion").minimumOrderQuantity).toBe(6);
    expect(sourceProduct("vaseline-body-care-series-200ml").variants?.map(v => v.unitPrice)).toEqual([3, 3, 3, 3, 3, 3]);
