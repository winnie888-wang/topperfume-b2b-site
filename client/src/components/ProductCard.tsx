...
export function ProductCard({ product, index }: { product: Product; index: number }) {
  ...
  return <article className={`product-card product-card-${product.category}`}>
    <Link href={`/products/${product.slug}`} className="product-image-wrap" aria-label={`View ${product.name}`}><div className="product-card-index">{String(index + 1).padStart(2, "0")}</div><img src={product.image} alt={`${product.category} authorised product visual`} className="product-image" /><span className="product-view">View Details <ArrowRight size={15} /></span></Link>
    <div className="product-meta"><div className="product-format"><span>{product.category}</span><span>{product.realImage ? "Authorized visual" : "Concept visual"}</span></div><Link href={`/products/${product.slug}`} className="product-name">{product.name}{product.nameStatus && <span className="product-name-status">{product.nameStatus}</span>}</Link><p className="product-type">{product.format}</p><div className="product-commercial"><span><b>Product Type</b>{product.descriptor}</span><span><b>Customizable</b>{customField}</span><span><b>Private Label Available</b>[TO CONFIRM]</span></div><div className="product-card-actions"><Link href={`/products/${product.slug}`} className="card-details-action">View Details <ArrowRight size={14} /></Link><InquiryDrawer triggerLabel="Request Sample" intent="sample" context={context} triggerClassName="card-sample-action" /></div></div>
  </article>;
}
