  ...
  return <Sheet open={open} onOpenChange={openDrawer}>
    ...
    <SheetContent side="right" className="inquiry-sheet">
      ...
      <form className="inquiry-form" onSubmit={handleSubmit} hidden={!displayState.showForm}>
          ...
          <p className="form-disclaimer">By submitting, you ask our team to use these contact details to respond to your inquiry. Product subtotals exclude shipping and taxes.</p>
        </form>
    </SheetContent>
  </Sheet>;
}
...
export function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="site-shell">
    ...
    <main>{children}</main>
    <footer className="site-footer"><div><Wordmark /><p>A trading company supporting wholesale beauty sourcing and supplier coordination. Custom project scope is confirmed with suppliers for each product.</p></div><div className="footer-index"><span>01 / Product browse</span><span>02 / Custom development</span><Link href="/contact">03 / Contact our team</Link></div><div className="footer-cta"><p>Choose a product. Request a sample. Start your project.</p><a className="footer-contact-link" href={`mailto:${businessProfile.email}`}>{businessProfile.email}</a><a className="footer-contact-link" href={`https://wa.me/${businessProfile.whatsappNumber}`} target="_blank" rel="noreferrer" onClick={() => trackWhatsAppCta("whatsapp", "Footer WhatsApp")}>WhatsApp {businessProfile.whatsappDisplay}</a><WhatsAppCta label="Customize / Private Label" /></div></footer>
  </div>;
}
