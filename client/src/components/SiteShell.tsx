import { OPEN_CONSENT } from "@/lib/consent";
...
const ConfirmedPerfumeTerms = createContext(false);
...
export function InquiryDrawer({ triggerLabel = "Start Your Project", intent = "project", context, triggerClassName = "" }: { triggerLabel?: string; intent?: InquiryIntent; context?: InquiryContext; triggerClassName?: string }) {
  const confirmedPerfumeTerms = useContext(ConfirmedPerfumeTerms);
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const summary = buildInquirySummary({ intent, confirmedPerfumeTerms, context: { ...context, subtotal: numericPrice ? `US$${(Number(numericPrice[1]) * Number(formValues.quantity)).toFixed(2)}` : undefined }, ...formValues });
    ...
    submitInquiry.mutate({
      ...
      onSuccess: (result) => {
        ...
        toast(confirmedPerfumeTerms ? "Inquiry submitted" : "Inquiry received", { description: "Your inquiry has been submitted to our team." });
      },
  ...
  return <Sheet open={open} onOpenChange={openDrawer}>
    ...
    <SheetContent side="right" className="inquiry-sheet">
      ...
      <div className="submission-note" role="status" aria-live="polite" hidden={!displayState.showSuccess}><span className="mini-index">01</span><h3>{confirmedPerfumeTerms ? "Thank you. Your inquiry has been submitted." : "Thank you. Your inquiry has been received."}</h3><p>Your submission was accepted for delivery. Reference: {submitted?.requestId}</p><div className="inquiry-route-status"><span>Company</span><strong>{businessProfile.companyName}</strong><span>Sales email</span><strong>{businessProfile.email}</strong><span>WhatsApp</span><strong>{businessProfile.whatsappDisplay}</strong></div><div className="inquiry-route-actions"><a className="button-secondary button-whatsapp" href={getInquiryWhatsAppUrl(submitted?.summary || buildInquirySummary({ intent, context, confirmedPerfumeTerms }))} target="_blank" rel="noreferrer" onClick={() => trackWhatsAppCta("whatsapp", "Continue on WhatsApp", context)}>Continue on WhatsApp</a></div><Button variant="outline" className="button-secondary" onClick={() => { submissionGate.current.completed = false; submitInquiry.reset(); setSubmitted(null); setFormStartedAt(Date.now()); }}>Submit another inquiry</Button></div>
      <form className="inquiry-form" onSubmit={handleSubmit} hidden={!displayState.showForm}>
          ...
          <div className="inquiry-two-up"><label>Quantity<Input required name="quantity" type="number" step="1" min={Number(context?.standardMoq?.match(/^\d+/)?.[0] ?? 1)} defaultValue={context?.quantity?.match(/^\d+/)?.[0] ?? ""} placeholder="Estimated units" /></label><label>Customization requirement<select name="customization" defaultValue="Not specified" aria-label="Customization requirement"><option value="Not specified">Not specified</option><option value="Branded Wholesale">Branded Wholesale</option>{(!confirmedPerfumeTerms || intent === "project") && <><option value="Private Label">Private Label</option><option value="OEM">OEM</option><option value="ODM">ODM</option></>}</select></label></div>
          ...
          <p className="form-disclaimer">By submitting, you ask our team to use these contact details to respond to your inquiry. Submitting does not subscribe you to marketing. <Link href="/privacy">Privacy notice</Link>. Product subtotals exclude shipping and taxes. {transactionGuidance}</p>
        </form>
    </SheetContent>
  </Sheet>;
}
...
export function WhatsAppAction({ context, className = "" }: { context?: InquiryContext; className?: string }) {
  ...
  function openWhatsApp() {
    ...
    const summary = buildInquirySummary({ intent: "whatsapp", confirmedPerfumeTerms, context: { ...context, productUrl: getCanonicalProductUrl(context?.productUrl) } });
...
export function WhatsAppCta({ label, intent = "project", context, className = "" }: { label: string; intent?: WhatsAppCtaIntent; context?: InquiryContext; className?: string }) {
  ...
  const href = getWhatsAppCtaUrl({ intent, confirmedPerfumeTerms, context: { ...context, productUrl } });
...
export function SiteShell({ children, confirmedPerfumeTerms = false }: { children: React.ReactNode; confirmedPerfumeTerms?: boolean }) {
  const [location] = useLocation();
  const [mobileOpen, setMobileOpen] = useState(false);
  return <ConfirmedPerfumeTerms.Provider value={confirmedPerfumeTerms}><div className="site-shell">
    ...
    <main>{children}</main>
    <footer className="site-footer"><div><Wordmark /><p>A trading company supporting wholesale beauty sourcing and supplier coordination. Custom project scope is confirmed with suppliers for each product.</p></div><div className="footer-index"><span>01 / Product browse</span><span>02 / Custom development</span><Link href="/contact">03 / Contact our team</Link><Link href="/contact#order-terms">Estimated dispatch &amp; order terms</Link><Link href="/buyer-guides">Buyer Guides</Link><Link href="/privacy">Privacy &amp; cookies</Link><button onClick={() => window.dispatchEvent(new Event(OPEN_CONSENT))}>Cookie preferences</button></div><div className="footer-cta"><p>Choose a product. Request a sample. Start your project.</p><a className="footer-contact-link" href={`mailto:${businessProfile.email}`}>{businessProfile.email}</a><a className="footer-contact-link" href={`https://wa.me/${businessProfile.whatsappNumber}`} target="_blank" rel="noreferrer" onClick={() => trackWhatsAppCta("whatsapp", "Footer WhatsApp")}>WhatsApp {businessProfile.whatsappDisplay}</a><WhatsAppCta label="Customize / Private Label" /></div></footer>
  </div></ConfirmedPerfumeTerms.Provider>;
}
