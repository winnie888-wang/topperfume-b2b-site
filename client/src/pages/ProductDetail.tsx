...
import { getDevelopmentScope, getProductDecisionRows, TO_CONFIRM } from "@/data/business";
...
export default function ProductDetail() {
  ...
  return <SiteShell>
    ...
    <section className={`pdp-hero pdp-${product.category}`}><div className="pdp-image-stage"><span className="pdp-index">01 / PRODUCT REFERENCE</span><img src={product.image} alt={`${product.category} authorised product visual`} /><span className="pdp-visual-tag">Authorized product visual / reference plate</span></div><div className="pdp-copy"><p className="eyebrow">{product.category} / reference format</p><h1>{product.name}</h1>{product.nameStatus && <span className="product-name-status">{product.nameStatus}</span>}<p className="pdp-descriptor">{product.descriptor}<em className="procurement-note">{TO_CONFIRM}</em></p><p className="pdp-intro">{product.briefing}</p><div className="pdp-tags">{product.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="pdp-cta-row"><InquiryDrawer triggerLabel="Request Sample" intent="sample" context={context} /><InquiryDrawer triggerLabel="Get Quote" intent="quote" context={context} triggerClassName="button-rose" /><WhatsAppAction context={context} /></div><p className="pdp-disclaimer">Product context is attached. MOQ, lead time and live routing remain <em className="procurement-note">{TO_CONFIRM}</em> until business information is approved.</p></div></section>
    <section className="decision-module"><div className="decision-module-copy"><p className="eyebrow">B2B DECISION MODULE</p><h2>Product details buyers need.</h2><p>Use these reference fields to prepare a sample or quote conversation. No operating information is represented as confirmed unless explicitly approved.</p><PackageOpen size={32} /></div><dl className="decision-grid">{decisionRows.map(({ label, value, status }) => <div key={label}><dt>{label}</dt><dd>{value}{status && <em className="procurement-note">{status}</em>}</dd></div>)}</dl></section>
    ...
    <section className="development-board"><div className="development-board-copy"><p className="eyebrow">CUSTOMIZATION SCOPE</p><h2>Make the product yours.</h2><p>Move from this product reference into the appropriate private-label, OEM or ODM conversation without assuming any unapproved capability.</p></div><div className="development-list"><p><Check size={19} /><span><strong>Product expression</strong>{getDevelopmentScope(product.category)}<em className="procurement-note">{TO_CONFIRM}</em></span></p><p><Check size={19} /><span><strong>Packaging &amp; logo</strong>Component, artwork, logo placement and carton scope.<em className="procurement-note">{TO_CONFIRM}</em></span></p><p><Check size={19} /><span><strong>Sample to quote</strong>Use the actions above with this exact product name, URL and category already attached.</span></p></div></section>
