...
export default function ProductDetail() {
  ...
  return <SiteShell>
    ...
    <section className={`pdp-hero pdp-${product.category}`}><div className="pdp-image-stage"><span className="pdp-index">01 / PRODUCT REFERENCE</span><img src={product.image} alt={`${product.name} product visual`} /><span className="pdp-visual-tag">{product.realImage ? "Client-provided product visual" : "Concept product visual"}</span></div><div className="pdp-copy"><p className="eyebrow">{product.category} / {product.format}</p><h1>{product.name}</h1><p className="pdp-descriptor">{product.descriptor}</p><p className="pdp-intro">{product.briefing}</p><div className="pdp-tags">{product.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="pdp-cta-row"><InquiryDrawer triggerLabel="Request Sample" intent="sample" context={context} /><InquiryDrawer triggerLabel="Get Quote" intent="quote" context={context} triggerClassName="button-rose" /><WhatsAppAction context={context} /></div><p className="pdp-disclaimer">Preview actions only. Live routing, MOQ and lead time are intentionally marked [TO CONFIRM].</p></div></section>
