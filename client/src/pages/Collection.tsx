...
const collectionCopy: Record<ProductCategory, { index: string; title: string; intro: string }> = {
  fragrance: { index: "01 / FRAGRANCE PRODUCTS", title: "Private Label Fragrance", intro: "Browse confirmed fragrance SKUs. Select a product to request a sample or quote." },
  skincare: { index: "02 / SKINCARE PRODUCTS", title: "Private Label Skincare", intro: "Browse confirmed body-care SKUs with key procurement details in view." },
  makeup: { index: "03 / MAKEUP PRODUCTS", title: "Private Label Makeup", intro: "Browse confirmed colour-makeup SKUs with product-specific sample and quotation paths." },
};
...
export default function Collection() {
  ...
  return <SiteShell>
    ...
    <section className="collection-body collection-body-fast"><div className="collection-decision-dossier"><p className="collection-dossier-index">01 / BUYER READINESS</p>{(["reference", "variable", "packaging", "commercial"] as const).map((field) => <div key={field}><b>{field === "reference" ? "Product Set" : field === "variable" ? "Customization" : field}</b><span>{buyerGuide[field].value}<em className="procurement-note">{buyerGuide[field].status}</em></span></div>)}</div><div className="collection-catalogue-break"><div><p>02 / PRODUCT CATALOGUE</p><h2>Compare confirmed SKUs.</h2></div><span>{visibleProducts.length} visible products</span></div><div className="product-grid collection-grid">{visibleProducts.map((product, index) => <ProductCard product={product} index={index} key={product.slug} />)}</div><aside className="collection-spine collection-spine-after-grid"><div className="spine-index">03 / BUYING NOTE</div><div className="spine-product"><strong>{leadProduct.name}</strong><span>Product format: {leadProduct.format}</span></div><div className="spine-field"><b>Sample</b>Product context included</div><div className="spine-field"><b>Customization</b>{customization}<em className="procurement-note">{TO_CONFIRM}</em></div><div className="spine-field"><b>Quote</b>Volume and market<em className="procurement-note">{TO_CONFIRM}</em></div><Link href={`/products/${leadProduct.slug}`} className="spine-link">View product details <ArrowRight size={14} /></Link></aside></section>
    <section className="collection-cta"><div><p className="eyebrow">NEED A DIFFERENT FORMAT?</p><h2>Move from product selection to quote.</h2><p>Standard orders start from 2 pcs with an approximate 7-day lead time and free samples available. Custom logo, packaging and fragrance requests start from 100 pcs; product-specific development terms remain {TO_CONFIRM} until confirmed.</p></div><InquiryDrawer triggerLabel="Get a Project Quote" intent="quote" context={{ category }} /></section>
